...
(define-struct bird_struct (x y yv) #:transparent)
(define-struct pipe_struct (x y) #:transparent)
(define-struct variables_struct (gravity momentum scroll-speed) #:transparent)
(define-struct state_struct (bird pipe variables list_pipe score)  #:transparent)
...
(define b (bird_struct bird-x bird-initial-y 0 ))
(define p (pipe_struct scene-width (+ added-number (random random-threshold))))
(define v (variables_struct initial-gravity initial-momentum initial-scroll-speed))
;(define l (list null))
...
(define (get-initial-state)
    (state_struct (bird_struct bird-x bird-initial-y 0 )
                  (pipe_struct scene-width (+ added-number (random random-threshold)))
                  (variables_struct initial-gravity initial-momentum initial-scroll-speed)
                  (list (pipe_struct scene-width (+ added-number (random random-threshold)))) 0)
  )
...
(define (next-state-bird bird gravity)
  (struct-copy bird_struct bird [yv (+ (bird_struct-yv bird) gravity)] [y (+ (bird_struct-y bird) (bird_struct-yv bird))])  )
...
(define (next-state-bird-onspace bird momentum)
  (struct-copy bird_struct bird [yv (- 0 momentum)] ))
...
(define (change current-state pressed-key)
  (match-let ([(state_struct bird pipe variables list_pipe score ) current-state])
    (cond [(key=? pressed-key pressed-key) (struct-copy state_struct current-state [bird (next-state-bird-onspace bird (variables_struct-momentum variables))])]     
          [else current-state]))
  )
...
(define (move-pipes pipes scroll-speed)
  (if (null? pipes)
      pipes
      (map (lambda (xul) (struct-copy pipe_struct xul [x (- (pipe_struct-x xul) scroll-speed)] ) ) pipes))
)
...
(define (clean-pipes pipes)
  (if (null? pipes)
      pipes
      (filter (lambda (pipe) (if (>= 0 (+ pipe-width (pipe_struct-x  pipe))) #f #t ) ) pipes ) 
  )
)
;(clean-pipes (repeater (lambda (pipes) (move-pipes pipes initial-scroll-speed)) initial-pipes 10000))
...
(define (add-more-pipes pipes)
  ;(if (null? pipes)
  ;  (cons  (pipe_struct (+ pipe-width pipe-gap) 0) pipes)
    (if (> no-pipes (length pipes)) 
    
    
      (cons (pipe_struct 
                      (+ (+ pipe-width pipe-gap) 
                         (pipe_struct-x (car  pipes))
                      )
...
(define (invalid-state? state)
  (if (check-pipe-collisions (state_struct-bird state) (state_struct-list_pipe state)) 
       #t
      (if (check-ground-collision (state_struct-bird state))
          #t
          #f
      )
  )
...
(define (check-pipe-collisions bird pipes)
  (if (null? pipes)
    #f
    (if (check-collision-rectangles (make-posn (pipe_struct-x (car pipes)) 0) 
                                    (make-posn (+ pipe-width (pipe_struct-x (car pipes))) (pipe_struct-y (car pipes))) 
                                    (make-posn (bird_struct-x bird) (bird_struct-y bird) ) 
                                    (make-posn (+ bird-width  (bird_struct-x bird)) (+ bird-height (bird_struct-y bird)) ))
        #t
        (if (check-collision-rectangles (make-posn (pipe_struct-x (car pipes)) (+ (pipe_struct-y (car pipes)) pipe-self-gap)) 
                                        (make-posn (+ pipe-width (pipe_struct-x (car pipes))) ground-y) 
                                        (make-posn (bird_struct-x bird) (bird_struct-y bird) ) 
                                        (make-posn (+ bird-width  (bird_struct-x bird)) (+ bird-height (bird_struct-y bird)) ))
            #t
...
(define (next-state state)
 (struct-copy state_struct state [bird  (next-state-bird (state_struct-bird state) (variables_struct-gravity (state_struct-variables state))  )] 
                                 [list_pipe (next-state-pipes  (state_struct-list_pipe state) (variables_struct-scroll-speed (state_struct-variables state)) ) ]
                                 [score  (+ 0.1 (state_struct-score state)) ])
...
(define (draw-frame state)
  ...
  (match-let ([(state_struct bird pipe variables list_pipe score)  state])
        (place-image bird-image (+ (bird_struct-x bird) (quotient bird-width 2)) (+ (bird_struct-y bird) (quotient bird-height 2))  
          (place-image ground-image (quotient scene-width 2) (+ ground-y (quotient ground-height 2)) 
            (place-image (score-to-image score) text-x text-y  
              (place-pipes list_pipe initial-scene)))
               )     
             )
)
; 
...
(define (place-pipes pipes scene)
     (foldl funcite_pipe scene pipes)                                                              
)


(define (funcite_pipe val scena)         
  (match-let ([(pipe_struct x y ) val])                     
              (place-image (rectangle pipe-width (abs (- scene-height (+ ground-height (+ y pipe-self-gap)))) "solid" "green") 
                                                    (+ x (quotient pipe-width 2)) 
                                                    (+ (+ pipe-self-gap y) (quotient (- ground-y (+ y pipe-self-gap )) 2) ) 
                                                    
                                                      (place-image (rectangle pipe-width y "solid" "green") 
                                                        (+ x (quotient pipe-width 2)) 
                                                        (- y (quotient y 2))                 
                                                        scena)
              )    
